...
interface AnthropicApiService {
    ...
    suspend fun getUsage(
        ...
        @Header("anthropic-beta") beta: String = "usage-2023-10-01",
...
class AnthropicApiClient {
    ...
    private val service: AnthropicApiService by lazy {
        Retrofit.Builder()
            ...
            .client(buildOkHttpClient(debug = false))
        ...
        return try {
            ...
            when {
                response.isSuccessful -> {
                    ...
                    if (body != null) {
                        val aggregated = AggregatedUsage.fromEntries(
                            entries = body.data,
                            periodStart = startDate,
                            periodEnd = endDate
                        )
                        ApiResult.Success(aggregated)
                    } else {
                        ApiResult.Error("Empty response body")
                    }
                }
                response.code() == 401 -> ApiResult.Error("Invalid API key. Please check your settings.", 401)
                response.code() == 403 -> ApiResult.Error("API key does not have usage access.", 403)
            ...
            ApiResult.Error("Request timed out. Check your network connection.")
        } catch (e: Exception) {
    ...
    suspend fun validateApiKey(apiKey: String): ApiResult<Boolean> {
        return try {
            ...
            when {
                ...
                response.code() == 403 -> ApiResult.Error("API key lacks usage permissions", 403)
